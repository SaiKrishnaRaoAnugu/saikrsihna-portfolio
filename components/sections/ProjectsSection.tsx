...
export default function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    if (n > 1) {
      tl.to(track, { xPercent: -((n - 1) / n * 100), ease: "none", duration: n - 1 }, 0);
      ...
      for (let i = 0; i < n - 1; i++) {
        ...
        if (curr) {
          tl.to(curr, { opacity: 0, y: -40, filter: "blur(6px)", duration: 0.2, ease: "power2.in" }, i + 0.3);
        }
        if (nextBg) {
          tl.fromTo(nextBg, { scale: 1.04 }, { scale: 1.0, duration: 1.0, ease: "power2.out" }, i);
        }
        if (next) {
          ...
          if (sub) tl.fromTo(sub, { y: 12, opacity: 0 }, { y: 0, opacity: 1, duration: 0.3, ease: "power2.out" }, i + 0.54);
          if (desc) tl.fromTo(desc, { y: 10, opacity: 0 }, { y: 0, opacity: 1, duration: 0.35, ease: "power2.out" }, i + 0.58);
          if (tags.length) tl.fromTo(tags, { y: 6, opacity: 0 }, { y: 0, opacity: 1, duration: 0.25, ease: "power2.out", stagger: 0.03 }, i + 0.65);
          if (btn) tl.fromTo(btn, { y: 8, opacity: 0 }, { y: 0, opacity: 1, duration: 0.3, ease: "power2.out" }, i + 0.72);
        }
      }
    }
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        if (n > 1) tl.progress(self.progress);
        const activeIdx = Math.round(self.progress * (n - 1));
        setSlideIdx((prev) => (prev !== activeIdx ? activeIdx : prev));
        if (progressRef.current) {
          gsap.set(progressRef.current, { scaleX: self.progress, transformOrigin: "left center", overwrite: true });
        }
        ...
        >
          {PROJECTS.map((proj, i) => (
            <div key={proj.id} className={styles.slide}>
              <div ref={(el) => { bgRefs.current[i] = el; }} className={styles.slideBg}>
                {proj.image ? (
                  <Image
                    src={proj.image}
                    alt={proj.title}
                  ...
                  <div className={styles.slideBgPlaceholder} />
                )}
              ...
              <div ref={(el) => { contentRefs.current[i] = el; }} className={styles.slideContent}>
